...
readonly final class ThumbnailsProcessor implements Processor
{
    ...
    {
        $post->setContent(strval($this->processThumbnails($post->getContent())));
        $post->setSummary($this->processThumbnails($post->getSummary()));
    }
    ...
    private function processThumbnails(?string $content): ?string
    {
        if ($content === null) {
            return $content;
        }
        ...
        for ($i = $thumbnailsMatched - 1; $i >= 0; $i--) {
            ...
            $replacement = sprintf('{{ thumbnails([ %s ]) }}', implode(",\n", $sources));
